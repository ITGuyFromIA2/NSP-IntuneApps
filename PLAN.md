...
The first downstream-only app wave is implemented as sanitized generic apps and generators; its remaining external step is the reviewed private Canon/HP artifact release. Disposable-machine validation for Parallels, DelegateService/SetACL, and AutoIt is now complete (see Phase 3 and [docs/TestVM.md](docs/TestVM.md)). The Phase 4 deployment executor's Create, UpdateContentInPlace, UpdateMetadataInPlace, and CreateSupersedingApp paths are all implemented and unit-tested, and (for Create) verified end to end against a real test tenant using VCred. Phase 3's remaining generator/runner items (Parallels guided configuration generator, engine-neutral schema-v2 runner) are both implemented and unit-tested now too - the schema-v2 runner's *output* still needs disposable-VM validation against a real installer before it drives a catalog app, same as everything else AutoIt-related in this repo. The exact handoff state, test evidence, and uncommitted-file warning are in [HANDOFF.md](HANDOFF.md).
...
- [x] Keep runtime window/process validation in the AutoIt/AHK execution layer when that remains the strongest implementation. `New-NSPInteractiveInstallerRunner -InstallerExecutablePath` has the generated script launch the installer itself, track its PID, and check `ProcessExists` before every step - a crashed or already-exited installer then fails with a specific "installer process is no longer running" message instead of surfacing as a generic window-not-found timeout on whatever step happens to run next. Optional (omit the parameter to keep the prior behavior of assuming the installer is already running, launched externally).
...
- [x] Implement the selected engine-neutral schema-v2 runner: a generic interpreter that drives AutoIt directly from a capture file. `New-NSPInteractiveInstallerRunner` validates a capture with `Test-NSPInteractiveInstallerCapture` and transpiles its `Steps` into a standalone `.au3` script, applying the lessons already learned in the disposable VM (an ID-based control selector over button text, per-step `Opt("WinTitleMatchMode", ...)`, bounded `WinWait`/`WinWaitClose` timeouts with `Optional` deciding fatal-vs-warning, `RuntimeParameter` values read from `NSP_RUNTIMEPARAM_<Name>` environment variables rather than ever being embedded in the script). **This transpiles correctly (unit-tested against both real example captures and synthetic ones covering every selector-fallback tier) but the emitted `.au3` output itself has not been run against a real installer or AutoIt runtime** - that needs the same disposable-VM pass documented in `docs/TestVM.md` before a generated script drives a real catalog app, exactly like the Notepad++ proof of concept received (whose one-off script was never itself committed to the repo - only its lessons, which this generator encodes).
...
- [x] Build the Parallels guided configuration generator so `ParallelsConnection.config.psd1` isn't hand-authored. `New-NSPParallelsClientApp` copies the existing, already-generic `Apps\ParallelsClient` Source/Detect implementation byte-for-byte (its `Detect_ParallelsClient.ps1` is Authenticode-signed - copied via `Copy-Item`, never rebuilt as a string literal, so the signature stays valid) and writes only the genuinely per-client inputs: a real `ParallelsConnection.config.psd1` and the instance's settings file. This was **not** actually the last blocker, despite the README's own framing - `Apps\ParallelsClient\README.md`'s checklist has three other items (disposable-VM resolver re-validation, comparing a real shared-device connection export, a version-aware detection contract) that need a live VM and a real Parallels client; those are untouched, and the reference app stays `RequiresRepair` until an operator finishes them.
...
- [x] Implement metadata PATCH while preserving assignments, dependencies, reporting history, and object ID. `Update-NSPIntuneWin32AppMetadata` (`PatchMetadata` stage) covers `Set-IntuneWin32App`'s display-metadata surface (DisplayName/Description/Publisher/featured flag) only - install experience, restart behavior, detection, and requirement rules are real Graph-PATCHable `win32LobApp` properties the community module has no cmdlet for, and this repo's classifier can't yet distinguish those changing from display text changing (both hash as `UpdateMetadataInPlace`). Documented as a known limitation in `docs/DeploymentEngine.md`, not silently guessed at.
- [x] Implement a new content-version upload/commit against the existing object.
- [x] Implement reviewed supersedence with graph-depth validation and explicit update-versus-replace choice. `Add-NSPIntuneWin32AppSupersedence` (`AddSupersedence` stage) walks the existing supersedence graph breadth-first before writing and refuses over Intune's 10-node cap; `Set-NSPAppDeploymentDecisions` asks the operator for Update-vs-Replace at plan-approval time (not guessed by the executor), carried through the run journal. Fixed a related bug found while wiring this in: `RecordManagementNotes` would have PATCHed the *superseded* app's Notes instead of no-op'ing, since it only special-cased `PlannedAction -eq 'Create'` and not `CreateSupersedingApp` (which also creates a fresh object via `CreateApp`).
- [x] Keep cleanup/retirement as a separate explicitly approved workflow. `Get-NSPIntuneAppRetirementCandidates` (read-only, requires the marker on both sides of the relationship) plus the existing `Remove-NSPIntuneWin32App` (plan-only, type-the-exact-name confirm) - dashboard `[17]`, deliberately separate from ad-hoc deletion at `[12]` and never triggered automatically by `AddSupersedence`.
- [x] Extend `Resolve-NSPAppBuildPlan` past `SetupType 'PoSH'`/`DetectionStyle 'Script'` only. Added `PoSH_sysnative` (Sysnative-redirected 64-bit PowerShell host, one consistent command-line shape for install/uninstall rather than Gen1's inconsistent dead-code version) and `Registry_Exist` (KeyPath/ValueName into `New-IntuneWin32AppDetectionRuleRegistry`). This was a real, previously-silent blocker: `Apps/DCU_DriverScan` is already catalog-classified `Deployable` and uses `PoSH_sysnative` - it would have thrown at the `Build` stage despite looking ready. `Apps/FortiClient_ImportConfig` (`Registry_Exist`, currently `RequiresConfiguration` pending real values) is unblocked the same way once configured.
...
- [x] Reuse suitable CLIBuilder/MasterOrchestrator and pushable-tools presentation conventions without coupling endpoint packages to those repositories. Ported `Get-DenseColumnLayout` from `NSP-FGTIPSecTools`' `IPSec-MasterOrchestrator.ps1` (confirmed pure/FortiGate-free by that repo's own Claude session) as `Get-NSPDenseColumnLayout`, plus two rendering helpers (`Write-NSPDenseFieldSummary` for the dashboard's many `$result | Format-List` previews, `Write-NSPDenseNumberedList` for the app-catalog pickers). `FieldMetadata.ps1` does not generalize - it's a static FortiGate/RADIUS field-grouping map, not a schema/prompt engine. The lettered sub-resume jump points ("1A", "1B" into a long guided flow) are a good pattern but have no current NSP-IntuneApps consumer long enough to need them; left for when one exists rather than inventing scope.
- [x] Keep domain functions callable non-interactively so `NSP-IntuneManager` can orchestrate them later. Audited every `Public/*.ps1` function for a `Read-Host`/`Read-NSPMenuChoice` call outside an `-Interactive`-gated block: every guided-template generator already gates all its prompts behind `-Interactive` (fully parameterizable without it), and `Start-NSPIntuneApps`/`Start-NSPInstallerCapture` are inherently interactive tools (a dashboard and a live capture recorder), not domain functions an orchestrator would call. The one real gap: `Set-NSPAppDeploymentDecisions` had no non-interactive path at all - it unconditionally prompted for every pending plan entry. Added `-Decisions` (an array of `{ Name; Decision; SupersedenceType }`, matched by entry name) as an alternative to the interactive per-entry loop; every pending entry must have a matching decision or it throws, rather than leaving one pending or guessing a default.
