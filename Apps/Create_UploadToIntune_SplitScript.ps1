throw 'This Gen1 uploader is intentionally disabled because it deletes and recreates Intune apps. Use Start-NSPIntuneApps.ps1 for discovery, validation, and deployment planning while the update-in-place/supersedence executor is completed.'

# Legacy implementation retained temporarily as migration reference below.
        ...
        if ($VariableConfig.DetectionStyle -eq "Script") {
            ...
            #Perform Signing
                foreach ($Sign in $VariableConfig.ToSign) {
                    $signature = Set-AuthenticodeSignature -FilePath $Sign.FullName -Certificate $codeCertificate -TimeStampServer $CodeSigningTimestampServer
                    if ($signature.Status -ne 'Valid') { throw "Signing failed for $($Sign.FullName): $($signature.StatusMessage)" }
                 }
        }
...
#foreach ($VariableConfig in $FinalCollection) {
    foreach ($Assignment in $VariableConfig.AssignmentColl) {
        foreach ($Group in $Assignment.GroupOIDs_Include) {
            ...
            $SplatMe = @{
                ID = $(($VariableConfig.IntuneWinApp.Win32App).id)
                Intent = $($Assignment.Intent) 
                Notification = $($Assignment.Nofication)
                DeliveryOptimizationPriority = $($Assignment.DeliveryOpt) 
            }

            if ($Group -ne "IntuneAllDevices" -and $Group -ne "IntuneAllUsers") {
                $SplatMe.GroupID = $Group
                $SplatMe.Include = $true
                
            }

            if ($Assignment.Filter.Name -ne "" -and $Assignment.Filter.Name -ne $null) {
                $SplatMe.FilterName = "$($Assignment.Filter.Name)" 
                $SplatMe.FilterMode = "$($Assignment.Filter.Mode)"
            
            }



            if ($Group -eq "IntuneAllDevices") {
                #AllDevices - W/o Filter
                Add-IntuneWin32AppAssignmentAllDevices @SplatMe

            } elseif ($Group -eq "IntuneAllUsers") {
               Add-IntuneWin32AppAssignmentAllUsers @SplatMe

            } else {

                Add-IntuneWin32AppAssignmentGroup @SplatMe
            }
        }
