...
$VariableConfig.VPNSettings.CompanyName = "Contoso"
$VariableConfig.VPNSettings.VPNAddress = "vpn.contoso.invalid"
$VariableConfig.VPNSettings.DNSSuffix = "contoso.invalid"
$VariableConfig.VPNSettings.UserTunnel = $true
$VariableConfig.VPNSettings.CAFilter = "CN=contoso-CA"
$VariableConfig.VPNSettings.DestPrefixes = @("192.168.1.15/32","192.168.1.16/32") 
$VariableConfig.VPNSettings.DeviceTask = $False
...
# Targeting is supplied by the deployment plan.
...
set-content -Path "$($TempBase)\Source\DownloadInstall_$($NameRepl)VPN.ps1" -Value $InstallFinal -Force
...
$($BaseScript)
"@
set-content -Path "$($TempBase)\Source\Uninstall_$($NameRepl)VPN.ps1" -Value $UninstallFinal -Force
...
$($BaseScript)
"@
set-content -Path "$($TempBase)\Detect\Detect_$($NameRepl)VPN.ps1" -Value $DetectFinal -Force
