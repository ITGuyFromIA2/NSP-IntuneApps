...
SetACL's publisher identifies the tool as freeware but requires a distribution license when it is distributed with another application. Direct retrieval from the publisher avoids embedding the executable in NSP source or an Intune package. The version remains fixed because service ACL behavior is infrastructure, not an evergreen end-user application. A disposable-VM audit must record the archive and executable hashes, file versions, archive paths, and Authenticode state before DelegateService can become deployable.
