function Start-NSPIntuneApps {
    ...
    .DESCRIPTION
        Two-level menu: a top-level category screen, each opening its own submenu of related
        actions. Replaces the earlier flat 20+ item list, whose numbering had already drifted out
        of order (items added over time landed wherever they'd fit, not where they belonged) and
        whose section headers no longer matched what was actually in them. Every action that
        existed before still exists - this is a regroup, not a cut - and every case body's actual
        logic is unchanged; only the surrounding navigation and in-body cross-references to other
        menu items (which no longer have stable global numbers) changed.

        Each submenu gets its own short status block using only already-loaded state or a cheap
        local file read (never a fresh Graph call just to render a menu) - the point of splitting
        into categories is exactly that there's now room to show category-relevant context instead
        of cramming everything into one shared header.
    #>
    [CmdletBinding()]
    param(
    ...
    $menuCategories = [ordered]@{
        '1' = @{
            ...
            Items = @(
                @{ Key = '1'; Id = 'Preflight'; Label = 'Preflight details' }
                @{ Key = '2'; Id = 'Catalog'; Label = 'App catalog' }
                @{ Key = '3'; Id = 'CreateTemplate'; Label = 'Create from a guided template' }
            )
        }
        '2' = @{
            Title = 'Code Signing'
            Items = @(
                @{ Key = '1'; Id = 'CodeSignStatus'; Label = 'Code-signing certificate status' }
                @{ Key = '2'; Id = 'PlanCertTrust'; Label = 'Plan certificate trust upload' }
            )
        }
        '3' = @{
            Title = 'Tenant Discovery'
            Items = @(
                @{ Key = '1'; Id = 'SaveAppInventory'; Label = 'Save a read-only Intune app inventory' }
                @{ Key = '2'; Id = 'SaveAssignmentInventory'; Label = 'Save a read-only app assignment and filter inventory' }
                @{ Key = '3'; Id = 'InstallStatus'; Label = 'View install status for a deployed app' }
            )
        }
        '4' = @{
            Title = 'Deployment Pipeline'
            Items = @(
                @{ Key = '1'; Id = 'CreateTracker'; Label = 'Create/review an app deployment tracker' }
                @{ Key = '2'; Id = 'ResumeTracker'; Label = 'Resume a saved deployment tracker' }
                @{ Key = '3'; Id = 'ViewRuns'; Label = 'View resumable deployment run journals' }
                @{ Key = '4'; Id = 'AdvanceRun'; Label = 'Advance the next stage of a saved run' }
            )
        }
        '5' = @{
            Title = 'Targeting & Filters'
            Items = @(
                @{ Key = '1'; Id = 'BuildFilter'; Label = 'Build and create an assignment filter' }
                @{ Key = '2'; Id = 'CookieCutterFilters'; Label = 'Deploy the standard cookie-cutter assignment filter set' }
                @{ Key = '3'; Id = 'MasterGroups'; Label = 'Manage master assignment groups (tenant defaults + per-app overrides)' }
            )
        }
        '6' = @{
            Title = 'App Actions'
        ...
        '7' = @{
            Title = 'Setup'
            Items = @(
                @{ Key = '1'; Id = 'Register'; Label = 'Register/verify the tenant app registration (one-time bootstrap)' }
            )
        }
    }
    ...
    $categoryChoice = $null
    do {
        $state = Get-NSPDashboardState -RepoRoot $RepoRoot
        ...
        foreach ($catKey in $menuCategories.Keys) {
            Write-Host ("  [{0}] {1}" -f $catKey, $menuCategories[$catKey].Title)
        }
        Write-Host ''
        Write-Host '  [Q] Quit'
        $categoryChoice = Read-NSPMenuChoice -Prompt 'Choose a category' -Allowed (@('0') + @($menuCategories.Keys) + 'Q')

        if ($categoryChoice -eq '0') {
            Write-NSPDashboardHeader -Title 'Manage the Graph connection' -StatusLines @($graphStatus)
            if ($graphContext) {
            ...
            Read-Host 'Press Enter to return to the dashboard' | Out-Null
        } elseif ($categoryChoice -ne 'Q') {
            $category = $menuCategories[$categoryChoice]
            $itemChoice = $null
            do {
                $state = Get-NSPDashboardState -RepoRoot $RepoRoot
                $preflight = $state.Preflight
                ...
                $latestInventoryFile = if (Test-Path -LiteralPath $inventoryRoot) { Get-ChildItem -LiteralPath $inventoryRoot -Filter '*.json' -File | Sort-Object LastWriteTimeUtc -Descending | Select-Object -First 1 } else { $null }
                $assignmentInventoryRootForStatus = Join-Path $RepoRoot '.nsp-intuneapps\assignment-inventory'
                $latestAssignmentInventoryFile = if (Test-Path -LiteralPath $assignmentInventoryRootForStatus) { Get-ChildItem -LiteralPath $assignmentInventoryRootForStatus -Filter '*.json' -File | Sort-Object LastWriteTimeUtc -Descending | Select-Object -First 1 } else { $null }
                $registrationPathForStatus = Join-Path $RepoRoot 'Config\Local\GraphAppRegistration.json'
                $registrationConfigured = Test-Path -LiteralPath $registrationPathForStatus

                $categoryStatusLines = switch ($categoryChoice) {
                    '1' {
                        @(
                            "Preflight: $(if ($preflight.Passed) { 'READY' } else { 'ATTENTION REQUIRED' })"
                    ...
                    '2' {
                        $codeSigningLine = try {
                            $csConfig = Get-NSPCodeSigningConfiguration -RepoRoot $RepoRoot
                            if ([string]::IsNullOrWhiteSpace([string]$csConfig.Current.Thumbprint)) { 'Active generation: none configured yet' }
                            else { "Active generation: $($csConfig.Current.Thumbprint) (expires $($csConfig.Current.NotAfter))" }
                        } catch { 'Active generation: not configured yet' }
                        @($codeSigningLine)
                    }
                    '3' {
                        @(
                            "App inventory: $(if ($latestInventoryFile) { "saved $($latestInventoryFile.LastWriteTime)" } else { 'none saved yet' })"
                            "Assignment/filter inventory: $(if ($latestAssignmentInventoryFile) { "saved $($latestAssignmentInventoryFile.LastWriteTime)" } else { 'none saved yet' })"
                        )
                    }
                    '4' {
                        @(
                    ...
                    '5' {
                        @(
                            "Tenant app registration: $(if ($registrationConfigured) { 'configured' } else { 'not configured - see Setup' })"
                            $graphStatus
                        )
                    }
                    '6' {
                        @(
                            $graphStatus
                            "App inventory: $(if ($latestInventoryFile) { "saved $($latestInventoryFile.LastWriteTime)" } else { 'none saved yet - see Tenant Discovery' })"
                        )
                    }
                    '7' {
                        if ($registrationConfigured) {
                            $reg = Get-Content -LiteralPath $registrationPathForStatus -Raw | ConvertFrom-Json
                            @("Registration: configured (tenant $($reg.TenantId))")
                        } else {
                            @('Registration: not configured yet')
                        }
                    }
                }

                Write-NSPDashboardHeader -Title $category.Title -StatusLines $categoryStatusLines
                foreach ($item in $category.Items) {
                    if ($item.Color) { Write-Host ("  [{0}] {1}" -f $item.Key, $item.Label) -ForegroundColor $item.Color }
                    else { Write-Host ("  [{0}] {1}" -f $item.Key, $item.Label) }
                }
                Write-Host ''
                Write-Host '  [B] Back'
                $itemChoice = Read-NSPMenuChoice -Prompt 'Choose an action' -Allowed (@($category.Items.Key) + 'B')

                if ($itemChoice -ne 'B') {
                    $selectedItem = $category.Items | Where-Object Key -eq $itemChoice
                    Write-NSPDashboardHeader -Title $selectedItem.Label -StatusLines @($graphStatus)
                    ...
                    switch ($selectedItem.Id) {
                        ...
                        'Catalog' { $catalog | Format-Table Name, Classification, ScriptCount, PackageCount, Reason -AutoSize -Wrap }
                        'CreateTemplate' {
                        ...
                        'InstallStatus' {
                            ...
                            if (-not $latestInventory) {
                                Write-Warning 'No saved Intune app inventory was found. Use Tenant Discovery > Save a read-only Intune app inventory first, so an app can be picked by name instead of a raw object ID.'
                        ...
                        'ViewRuns' {
                            if ($savedRuns.Count -eq 0) {
                                ...
                                Write-Host 'Use Deployment Pipeline > Advance the next stage of a saved run to advance a run one stage at a time, or auto-advance every remaining stage for testing. UpdateMetadataInPlace and CreateSupersedingApp stages are not yet implemented.' -ForegroundColor Yellow
                            }
                        }
                        'AdvanceRun' {
                            if ($savedRuns.Count -eq 0) {
                        ...
                        'AssignApp' {
                            ...
                            if (-not (Test-Path -LiteralPath $registrationPath)) {
                                ...
                                Write-Warning 'No saved Intune app inventory was found. Use Tenant Discovery > Save a read-only Intune app inventory first, so an app can be picked by name.'
                            } else {
                                $registration = Get-Content -LiteralPath $registrationPath -Raw | ConvertFrom-Json
                                $inventoryDocument = Get-Content -LiteralPath $latestInventory.FullName -Raw | ConvertFrom-Json
            ...
            } while ($itemChoice -ne 'B')
        }
    } while ($categoryChoice -ne 'Q')
}
